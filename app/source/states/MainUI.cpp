...
Result MainUI::switchAccounts(MainStruct *mainStruct, u8 friend_account_id) {
	...
	handleResult(FRDA_SetLocalAccountId(friend_account_id), mainStruct, "Switch account");
	if (rc) {
		return rc;
	}
	...
	if (act_account_index == 0) {
		handleResult(ACTA_CreateLocalAccount(), mainStruct, "Create act account");
		handleResult(ACTA_GetAccountIndexOfFriendAccountId(&act_account_index, friend_account_id), mainStruct,
...
bool MainUI::drawUI(MainStruct *mainStruct, C3D_RenderTarget* top_screen, C3D_RenderTarget* bottom_screen, u32 kDown, u32 kHeld, touchPosition touch)
{
    ...
    if (mainStruct->buttonWasPressed) {
        ...
        Result rc = switchAccounts(mainStruct, accountId);
        if (rc && accountId == 2) {
